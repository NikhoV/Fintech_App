...
const Conto_Details = () => {
  ...
  useEffect(() => {
    if (id_conto) {
      setLoading(true);
      Promise.all([fetchAccount(id_conto), fetchMovimenti(id_conto)]).finally(
        () => setLoading(false),
      );
    }
  }, [id_conto]);
    ...
    <div className="container py-4">
      ...
      {account && (
        <>
          ...
          {/* Balance Card */}
          <div
            className="card card-gradient mb-6"
          ...
          >
            ...
            >
              {account.valuta}{" "}
              {parseFloat(account.saldo).toLocaleString("it-IT", {
                ...
                {movimenti.map((movimento) => {
                  ...
                  return (
                    <div key={movimento.id_spesa} className="transaction-item">
                      <div className="flex gap-3">
                        <div
                          className={`transaction-icon ${isPositive ? "income" : "expense"}`}
                        >
                          {isPositive ? (
                      ...
                      <p
                        className={`transaction-amount ${isPositive ? "income" : "expense"}`}
                      >
                        {isPositive ? "+" : ""}€
